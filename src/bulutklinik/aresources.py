"""Asynchronous service resources — mirror of :mod:`bulutklinik.resources`."""
...
# See resources.py: the `list` method shadows builtin `list` inside the measures
# resource, so `list[...]` annotations there use this module-level alias.
...
class AsyncAuthResource:
    ...
    async def connect(
        ...
        *,
        ...
        data = await self._http.send(RequestSpec("POST", "/general/connectApi", "public", body))
        return finish_login(data, self._http.token_store)

    async def connect_with_two_factor(self, sms_verification_code: str, response: str) -> None:
    ...
    async def register(
        ...
        *,
        ...
        data = await self._http.send(RequestSpec("POST", "/patients/addNewPatient", "public", body))
        store_tokens(data, self._http.token_store)
    ...
    async def verify_registration_social(
        self,
        *,
        name: str,
        surname: str,
    ...
    async def reset_password(
        ...
        *,
        ...
        return await self._http.send(_spec.reset_password(sms_confirm_code, response, password))
    ...
    async def disconnect(self) -> None:
        try:
            await self._http.send(RequestSpec("POST", "/general/disconnectApi", "bearer", {}))
        finally:
...
class AsyncDoctorsResource:
    ...
    async def quick_search(
        ...
        return await self._http.send(_spec.quick_search(search_text, list_type, location))
    ...
    async def detail(self, doctor_id: int | str, corporate: int | str | None = None) -> Any:
        return await self._http.send(_spec.doctor_detail(doctor_id, corporate))
...
class AsyncAppointmentsResource:
    ...
    async def add_physical(self, doctor_id: int | str, appointment_date: str) -> Any:
        return await self._http.send(_spec.add_physical(doctor_id, appointment_date))

    async def cancel(self, event_id: int | str) -> Any:
        return await self._http.send(_spec.cancel_appointment(event_id))
    ...
    async def list(self, page: int | str | None = None) -> Any:
        ...
        """
        return await self._http.send(_spec.user_appointments(page))

    async def reservations(self) -> Any:
        """The patient's active online-slot reservation holds (with a ``minute_diff`` countdown)."""
        return await self._http.send(_spec.user_reservations())
...
class AsyncAddressesResource:
    ...
    async def list(self) -> Any:
        ...
        return await self._http.send(_spec.address_list())
    ...
    async def delete(self, address_id: int | str) -> Any:
        """Delete an address by id (default/used addresses cannot be deleted)."""
        return await self._http.send(_spec.address_delete(address_id))
...
class AsyncPaymentsResource:
    ...
    async def get_cards(self) -> Any:
        return await self._http.send(_spec.get_cards())
    ...
    async def delete_card(self, card_id: int | str) -> Any:
        return await self._http.send(_spec.delete_card(card_id))
...
class AsyncMeasuresResource:
    ...
    async def add_list(self, records: _MeasureRecords) -> Any:
        return await self._http.send(_spec.add_measures(records))

    async def add(self, measure_type: str, fields: dict[str, Any]) -> Any:
        return await self._http.send(_spec.add_measure(measure_type, fields))

    async def update(self, measure_type: str, fields: dict[str, Any]) -> Any:
        return await self._http.send(_spec.update_measure(measure_type, fields))

    async def delete(self, measure_type: str, measure_id: int | str) -> Any:
        return await self._http.send(_spec.delete_measure(measure_type, measure_id))

    async def last(self) -> Any:
        return await self._http.send(_spec.measures_last())

    async def list(
        self, measure_type: str, page: int | str, glucose_type: int | None = None
    ) -> Any:
        return await self._http.send(_spec.measures_list(measure_type, page, glucose_type))

    async def graph(
        self, measure_type: str, period: int, page: int | str, glucose_type: int | None = None
    ) -> Any:
        return await self._http.send(_spec.measures_graph(measure_type, period, page, glucose_type))

    async def partner_health_information(
        self, *, identity: str | None = None, phone_number: str | None = None, data: _MeasureRecords
    ) -> Any:
        return await self._http.send(_spec.partner_health_information(identity, phone_number, data))


class AsyncSkinResource:
    """AI skin-lesion analysis ("Cildimde Neyim Var")."""

    def __init__(self, http: AsyncHttpClient) -> None:
        self._http = http

    async def analyze(self, images: list[dict[str, Any]]) -> Any:
        ...
        ``caseDetail``.
        """
        return await self._http.send(_spec.image_check(images))


class AsyncMealsResource:
